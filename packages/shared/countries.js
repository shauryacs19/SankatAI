// Country list for phone-number pickers: { name, code (ISO-3166 alpha-2),
// dial (calling-code prefix, no +) }. Flag emoji is derived from the code.
// Framework-agnostic — shared by web and mobile.
...
export const COUNTRIES = RAW.map(([name, code, dial]) => ({ name, code, dial, flag: flagEmoji(code) }))

export const DEFAULT_COUNTRY = COUNTRIES.find((c) => c.code === 'IN') || COUNTRIES[0]

// Best-effort: pick the country whose dial code prefixes a stored "+<dial> <num>".
export const countryFromPhone = (phone) => {
  const digits = String(phone || '').replace(/[^\d]/g, '')
  if (!digits) return null
  const matches = COUNTRIES.filter((c) => digits.startsWith(c.dial))
  if (!matches.length) return null
  return matches.reduce((a, b) => (b.dial.length > a.dial.length ? b : a))
}

// Strip the leading dial code from a stored phone to get the local number.
export const localNumber = (phone, country) => {
  const digits = String(phone || '').replace(/[^\d]/g, '')
  if (country && digits.startsWith(country.dial)) return digits.slice(country.dial.length)
  return digits
}
