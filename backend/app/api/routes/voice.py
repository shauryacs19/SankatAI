...
Flow:
  1. POST /api/voice/session {languageCode} -> { url, expiresIn, ... }
  2. client opens `url` (wss, 60s to connect) and sends PCM as AWS event-stream
  3. Transcribe returns partial/final transcripts to the client
  4. client sends the final text through the normal chat messages endpoint
"""
...
from app.services.voice_service import VoiceError, VoiceRateLimitError, VoiceUnavailableError

router = APIRouter(prefix="/api/voice", tags=["Voice"])


@router.post("/session", response_model=VoiceSessionResponse, summary="Get a presigned Transcribe streaming URL")
def create_session(response: Response, body: VoiceSessionRequest | None = None, user: CurrentUser = Depends(get_current_user)):
    # The URL carries signing credentials: never let any cache keep it.
    response.headers["Cache-Control"] = "no-store"
    try:
        return voice_service.create_session(user.user_id, (body or VoiceSessionRequest()).languageCode)
